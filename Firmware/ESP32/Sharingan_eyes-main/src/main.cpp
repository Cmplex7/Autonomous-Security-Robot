...
***************************************************************/
#define USE_DMA
#define AUTO_SWITCH_INTERVAL 5000 
...
// Cac loi mat (iris) doi duoc luc chay bang nut bam.
// Loi co scaleMin == scaleMax == 120 la loi "phang": texture phu kin
// toan bo vong iris (kieu Sharingan), khong gian/co dong tu.
typedef struct
{
    const uint16_t *map; // texture iris 256x64 (polar map)
    uint16_t scaleMin;   // iScale khi dong tu gian to nhat
    uint16_t scaleMax;   // iScale khi dong tu co nho nhat
} irisStyle_t;

// iScale 121 (KHONG dung 120): pixel ngoai vong tron iris mang ma polar
// dist=127, can 121*127/240 = 64 >= IRIS_MAP_HEIGHT de roi ra sclera;
// voi 120 chung roi vao hang 63 cua texture lam iris bi VUONG.
static const irisStyle_t irisStyles[] = {
    { iris,          IRIS_MIN, IRIS_MAX }, // mat nau mac dinh
    { iris_tomoe,    121, 121 },           // Sharingan tomoe
    { iris_kamui,    121, 121 },           // Mangekyou Kamui
    { iris_sasuke,   121, 121 },           // Mangekyou vinh hang (Sasuke)
    { iris_rinnegan, 121, 121 },           // Rinnegan
};
#define NUM_IRIS_STYLES (sizeof(irisStyles) / sizeof(irisStyles[0]))

static const char *irisStyleNames[] = {
    "mat nau", "Sharingan tomoe", "Mangekyou Kamui", "Mangekyou Sasuke", "Rinnegan",
};

// Khoi dong thang vao loi Sasuke (index 3) de kiem tra
static uint8_t irisStyleIndex = 3;
static const uint16_t *curIris = iris_sasuke;
static uint16_t curIrisMin = 121, curIrisMax = 121;

// Doc nut doi loi mat. Phai goi tu frame() (moi khung hinh) vi split()
// chan loop() lien tuc ca chuc giay.
static void checkIrisButton(void)
{
    static uint32_t lastSwitch = 0;

    // --- Nut bam thu cong (neu co) ---
#if defined(IRIS_SWITCH_PIN) && (IRIS_SWITCH_PIN >= 0)
    static bool lastState = HIGH;
    static uint32_t lastEdge = 0;

    bool s = digitalRead(IRIS_SWITCH_PIN);
    if (s != lastState && millis() - lastEdge > 200)
    {
        lastEdge = millis();
        lastState = s;
        if (s == LOW)
        {
            irisStyleIndex = (irisStyleIndex + 1) % NUM_IRIS_STYLES;
            curIris    = irisStyles[irisStyleIndex].map;
            curIrisMin = irisStyles[irisStyleIndex].scaleMin;
            curIrisMax = irisStyles[irisStyleIndex].scaleMax;
            lastSwitch = millis(); // Reset timer khi bam nut
            Serial.print("Doi loi mat (nut) -> ");
            Serial.println(irisStyleNames[irisStyleIndex]);
        }
    }
#endif

    // --- Tu dong doi theo thoi gian ---
    if (millis() - lastSwitch >= AUTO_SWITCH_INTERVAL)
    {
        lastSwitch = millis();
        irisStyleIndex = (irisStyleIndex + 1) % NUM_IRIS_STYLES;
        curIris    = irisStyles[irisStyleIndex].map;
        curIrisMin = irisStyles[irisStyleIndex].scaleMin;
        curIrisMax = irisStyles[irisStyleIndex].scaleMax;
        Serial.print("Doi loi mat (tu dong) -> ");
        Serial.println(irisStyleNames[irisStyleIndex]);
    }
}
...
#if defined(IRIS_SWITCH_PIN) && (IRIS_SWITCH_PIN >= 0)
    pinMode(IRIS_SWITCH_PIN, INPUT_PULLUP); // Nut doi loi mat
#endif
...
static void updateEye(void)
{
    ...
    newIris = (curIrisMin < curIrisMax) ? random(curIrisMin, curIrisMax)
                                        : curIrisMin;
    split(oldIris, newIris, micros(), 10000000L, curIrisMax - curIrisMin);
    oldIris = newIris;
        ...
        for (screenX = 0; screenX < SCREEN_WIDTH; screenX++, scleraX++, irisX++, lidX += dlidX)
        {
            ...
            {                                                          // Maybe iris...
                ...
                {                                                     // Within iris area
                    ...
                    p = pgm_read_word(curIris + d * IRIS_MAP_WIDTH + a); // Pixel = iris
                }
                else
                {                                                                 // Not in iris
...
static void frame(uint16_t iScale)
{
    ...
    checkIrisButton(); // Nut doi loi mat

    // Loi mat "phang" (Sharingan): iScale co dinh, bo qua gia tri tu split()
    if (curIrisMin == curIrisMax)
    {
        iScale = curIrisMin;
    }
...
{ 
    ...
    {               // No more subdivisons, do iris motion...
        ...
        while ((dt = (micros() - startTime)) < duration)
        {
            ...
            if (v < curIrisMin)
            {
                v = curIrisMin; // Clip just in case
            }
            else if (v > curIrisMax)
            {
                v = curIrisMax;
            }
